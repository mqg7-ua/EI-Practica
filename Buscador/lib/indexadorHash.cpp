...
string IndexadorHash::TransformarTerm(const string& word) const {
    ...
    string terminoProcesado = tokensBusqueda.front(); //solo estoy mirando una palabra, estara en .front()
    stemmerPorter miStemmer;
...
bool IndexadorHash::Existe(const string& word) const {
    string term = TransformarTerm(word); //le aplico el tokenizado...
    if(term.empty()) return false;
    return (indice.find(term) != indice.end());//si .find() no lo encuentra devuelve .end()
}
...
bool IndexadorHash::GuardarIndexacion() const {
    ...
    //GUARDAR ÍNDICE DE DOCUMENTOS (indiceDocs)
    ...
    //GUARDAR ÍNDICE PRINCIPAL (indice)
    ...
    for (unordered_map<string, InformacionTermino>::const_iterator it = indice.begin(); it != indice.end(); ++it) {
        ...
        for (unordered_map<int, InfTermDoc>::const_iterator itDocs = it->second.l_docs.begin(); itDocs != it->second.l_docs.end(); ++itDocs) {
            ...
            // Si las posiciones estan guardadas, iteramos la estructura
            if (almacenarPosTerm) {
                for (vector<int>::const_iterator itPos = itDocs->second.posTerm.begin(); itPos != itDocs->second.posTerm.end(); ++itPos) {
                    fInd << *itPos << " ";
                }
            }
            fInd << '\n';
    ...
    //sGUARDAR ÍNDICE DE LA PREGUNTA (Si hay alguna)
    ...
    if (!pregunta.empty()) {
        ...
        for (unordered_map<string, InformacionTerminoPregunta>::const_iterator it = indicePregunta.begin(); it != indicePregunta.end(); ++it) {
            ...
            if (almacenarPosTerm) {
                // Cambiamos list<int> por vector<int>
                for (vector<int>::const_iterator itPos = it->second.posTerm.begin(); itPos != it->second.posTerm.end(); ++itPos) {
                    fPreg << *itPos << " ";
                }
            }
            fPreg << '\n';
...
bool IndexadorHash::RecuperarIndexacion(const string& directorioIndexacion) {
    ...
    //CARGAR ÍNDICE PREGUNTA (Solo si existe)
    string rutaPreg = directorioIndice + "/pregunta.txt";
